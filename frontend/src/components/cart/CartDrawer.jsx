import { ArrowRight, Minus, Plus, Scissors, ShoppingBag, Trash2, Truck, X } from 'lucide-react';
import { useDispatch, useSelector } from 'react-redux';
import { useNavigate } from 'react-router-dom';
import { AnimatePresence, motion } from 'framer-motion';
import { removeItem, toggleCart, updateQty } from '../../redux/slices/cartSlice';
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto px-5 py-5">
                {items.length === 0 ? (
                  ...
                  <div className="grid gap-4">
                    {items.map((item) => (
                      <article key={item.id} className="grid grid-cols-[88px_1fr] gap-4 rounded-[8px] border border-[#DDE4EA] bg-white p-3 shadow-xs">
                        <div className="h-28 overflow-hidden rounded-[8px] bg-[#EEF3F6]">
                          <img
                            src={item.image || 'https://images.unsplash.com/photo-1610030469983-98e550d6193c?auto=format&fit=crop&fm=webp&w=240&q=80'}
                            alt={item.name}
              ...
              {items.length > 0 && (
                <footer className="border-t border-[#DDE4EA] bg-[#F8FAFC] p-5">
                  ...
                  <button onClick={handleCheckout} className="sk-button-primary mt-4 w-full">
                    Checkout
