...
```mermaid
flowchart TD
    Lead["Incoming Customer Enquiry"] --> Seg{"Identify Context & Need"}
    Seg -->|Wedding / Trousseau| S1["Segment A: Bridal / Wedding\nFocus: Heirlooms, Weave Authenticity, Venue Lighting"]
    Seg -->|Guest / Puja / Function| S2["Segment B: Wedding Guest / Festive\nFocus: Drape Comfort, Photogenic Styling, Ease of Movement"]
    Seg -->|Anniversary / Milestone| S3["Segment C: Premium Gift\nFocus: Unboxing Presentation, Emotional Value, Storytelling"]
    Seg -->|Corporate Gifting / Farewell| S4["Segment D: Corporate Gifting\nFocus: Invoice Transparency, Budget Adherence, Neutral Sizing"]
    Seg -->|Textile Purist / Collector| S5["Segment E: Handloom Purist\nFocus: Loom Provenance, Yarn Count, GI Verification"]
```
...
sequenceDiagram
    ...
    actor Stylist as SareeKart Stylist
    Customer->>Stylist: Initial Inquiry ("Hi, interested in sarees")
    Stylist->>Customer: Acknowledge & Question 1 (Occasion)
    Customer->>Stylist: "Attending sister's wedding in December"
    Stylist->>Customer: Question 2 & 3 (Color & Fabric Preference)
    Customer->>Stylist: "Looking for rich jewel tones, comfortable silk"
    Stylist->>Customer: Share 2–3 Curated Drapes (Specs, High-Res Images, Video Link)
    Customer->>Stylist: Inquire on Specific Drape ("Tell me more about SKU #4")
    Stylist->>Customer: Provide Detailed Loom Story & Answer Questions Factual
    Customer->>Stylist: Customer Chooses Drape ("I will take this one")
    Stylist->>Customer: Share Order Details & Payment Instructions (Bank/UPI)
    Customer->>Stylist: Sends UTR / Payment Evidence
    Stylist->>Customer: Verify Bank Settlement -> Confirm Order -> Share QC & Dispatch Timeline
```
...
```mermaid
flowchart LR
    E["1. Enquiries"] --> R["2. Replies"]
    R --> Q["3. Qualified"]
    Q --> S["4. Product Shared"]
    S --> C["5. Checkout Started"]
    C --> P["6. Payment Settled"]
    P --> D["7. QC & Delivered"]
    D --> Rep["8. Repeat / Referral"]
```
