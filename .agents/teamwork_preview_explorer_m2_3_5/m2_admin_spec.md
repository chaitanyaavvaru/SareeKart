# Milestone 2 Architectural Specification: Admin Moderation Console & Navigation
**Target Files**: 
- `frontend/src/pages/Admin/ManageReturns.jsx` (New)
- `frontend/src/routes/AppRouter.jsx` (Modified)
- `frontend/src/pages/Admin/AdminDashboard.jsx` (Modified)

**Document Reference**: `teamwork_preview_explorer_m2_3_5/m2_admin_spec.md`  
**Author**: Explorer 3 (Milestone 2)  
**Status**: Specification Complete — Ready for Downstream Implementation  
...
In SareeKart v3.0 Module 1, customers are provided with self-service return and exchange requests for delivered orders within 7 calendar days. To complete the reverse logistics loop, store operations staff (`OWNER`, `MANAGER`, `ADMIN`) require a dedicated administrative moderation console.

This specification establishes the blueprint for:
1. **Admin Moderation Console (`ManageReturns.jsx`)**:
   - 4 responsive KPI metric cards: Total Claims, Pending Review, Pickups Scheduled, Completed Refunds.
   - Filterable claims table with status tabs (`ALL`, `PENDING`, `APPROVED`, `PICKUP_SCHEDULED`, `REJECTED`, `COMPLETED`), multi-parameter search, customer telemetry, financial amounts, reason taxonomy, and defect photo thumbnails.
   - High-resolution defect photo inspection drawer / lightbox with side-by-side claim context and photo switcher.
   - 1-Click action controls enforcing backend state transitions:
     - **Approve Return**: Advances status from `PENDING` to `APPROVED`.
     - **Assign Courier & Schedule Pickup**: Interactive modal with courier dropdown (Blue Dart, Delhivery, DTDC, India Post), auto-generated/scanned AWB tracking code, and intake instructions; advances status from `APPROVED` to `PICKUP_SCHEDULED`.
     - **Complete Refund**: Marks physical inspection verified and refund disbursed; advances status from `PICKUP_SCHEDULED` to `COMPLETED`.
     - **Reject Return**: Modal enforcing mandatory rejection explanation via quick policy chips and detailed notes; advances status to `REJECTED`.
2. **Navigation & Routing Integration**:
   - Lazy-loaded route `/admin/returns` in `frontend/src/routes/AppRouter.jsx` under `ProtectedRoute adminOnly={true}`.
   - "Returns & Exchanges" sidebar entry under the `Commerce` group in `frontend/src/pages/Admin/AdminDashboard.jsx` using the `RotateCcw` icon.
3. **Bundle Budget & Offline Discipline**:
   - Zero external chart or lightbox dependencies. Pure Tailwind CSS v4 and Lucide React SVG icons.
   - Projected bundle chunk size < 25 kB (strict budget: < 500 kB).
   - Built-in resilient offline mock data fallback for immediate preview and testing.

---

## 2. Role-Based Access Control (RBAC)

### 2.1 Route-Level Protection
- Mounted inside `<Route path="/admin" element={<ProtectedRoute adminOnly={true}><AdminDashboard /></ProtectedRoute>}>` in `AppRouter.jsx`.
- As verified in `frontend/src/components/common/ProtectedRoute.jsx`:
  ```javascript
  allowedRoles = ['ADMIN', 'OWNER', 'MANAGER']
  ```
  Unauthenticated visitors are redirected to `/login?redirect=/admin/returns`.
  Authenticated customers (`role === 'CUSTOMER'`) are redirected to `/` with no access to admin routes.

### 2.2 Component-Level Role Awareness
- Inside `ManageReturns.jsx`, retrieve the user profile from Redux state:
  ```javascript
  const { user } = useSelector((state) => state.auth);
  const isStaff = ['OWNER', 'MANAGER', 'ADMIN'].includes(user?.role);
  ```
- If an unauthorized request reaches the backend, `AdminReturnController.java` returns HTTP 403 Forbidden with:
  ```json
  { "success": false, "message": "Not authorised to perform this action" }
  ```
- The frontend gracefully catches this response and displays an actionable alert banner.

---

## 3. Navigation & Routing Integration

### 3.1 Route Registration in `frontend/src/routes/AppRouter.jsx`
1. Add the lazy import alongside other admin pages:
   ```javascript
   const ManageReturns = lazy(() => import('../pages/Admin/ManageReturns'));
   ```
2. Add the child route inside the protected admin layout:
   ```jsx
   <Route 
     path="/admin" 
     element={
...
### 3.2 Sidebar Navigation in `frontend/src/pages/Admin/AdminDashboard.jsx`
1. Import `RotateCcw` from `lucide-react`:
   ...
   ```
2. Add the item to `ADMIN_NAV` under the `Commerce` group:
   ```javascript
   const ADMIN_NAV = [
...
|---|---|---|---|
| `GET` | `/api/admin/returns` | Fetch all claims or filter by status | Query param `?status=ALL\|PENDING\|APPROVED\|PICKUP_SCHEDULED\|REJECTED\|COMPLETED` |
| `PUT` | `/api/admin/returns/{id}/status` | Update claim status & reverse logistics | JSON body (`ReturnStatusUpdateRequest`) |

### 4.2 API Payload Structures

#### 1. Status Update: Approve Return
```json
PUT /api/admin/returns/101/status
{
  "status": "APPROVED",
  "adminNotes": "Preliminary photo review approved for reverse pickup."
}
```

#### 2. Status Update: Assign Courier & Schedule Pickup
```json
PUT /api/admin/returns/101/status
{
  "status": "PICKUP_SCHEDULED",
  "reverseCourier": "Blue Dart Reverse Logistics",
  "reverseTrackingNumber": "BDR-RET-37-9021",
  "adminNotes": "Pickup scheduled. Keep saree in original packaging with tags."
}
```
*Note*: `reverseCourier` and `reverseTrackingNumber` are strictly validated by backend `ReturnServiceImpl.java:303`.

#### 3. Status Update: Complete Refund
```json
PUT /api/admin/returns/101/status
{
  "status": "COMPLETED",
  "refundAmount": 18500.00,
  "adminNotes": "Item received at Bengaluru central hub, inspection verified, refund disbursed."
}
```

#### 4. Status Update: Reject Return
```json
PUT /api/admin/returns/101/status
{
  "status": "REJECTED",
  "adminNotes": "Defect not substantiated by submitted photographs. Zari weaving is consistent with master craft standards."
}
```
*Note*: `adminNotes` is strictly mandatory for `REJECTED` per `ReturnServiceImpl.java:285`.

### 4.3 `ReturnResponse` Telemetry Structure
```typescript
interface ReturnClaim {
  ...
  refundAmount: number;
  refundMode: 'ORIGINAL_PAYMENT' | 'STORE_CREDIT' | 'EXCHANGE_DRAPE';
  type: 'RETURN' | 'EXCHANGE';
  reason: 'COLOR_MISMATCH' | 'ZARI_DEFECT' | 'FABRIC_FEEL' | 'INCORRECT_ITEM' | 'SIZE_MISMATCH' | 'OTHER';
  comments: string;
  status: 'PENDING' | 'APPROVED' | 'PICKUP_SCHEDULED' | 'REJECTED' | 'COMPLETED';
...
## 5. UI/UX Architecture of `ManageReturns.jsx`

### 5.1 Design Tokens & Theme Alignment
The component adheres strictly to the SareeKart artisan palette:
- Primary Dark: `#17211F` (header, active tabs, buttons)
- Primary Teal: `#1E6A62` (accents, links, icons)
- Light Mint: `#E3F0ED` (badges, selected pill backgrounds)
- Gold Accent: `#F3C56A` (active indicator, badges)
- Canvas Cream: `#F7F4EE` (background)
- Card Border: `#DDD8CF`
- Muted Slate: `#71817A`

---

### 5.2 4-Card KPI Metric Summary Bar

A responsive 4-column grid (`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6`):

```
┌───────────────────────────┐ ┌───────────────────────────┐ ┌───────────────────────────┐ ┌───────────────────────────┐
│ Total Claims lodged       │ │ Pending Review            │ │ Pickups Scheduled         │ │ Completed Refunds         │
│ Count: 24                 │ │ Count: 6                  │ │ Count: 8                  │ │ Count: 10                 │
│ Value: ₹3,42,000          │ │ Priority: Urgent          │ │ Transit: In Reverse Hub   │ │ Disbursed: ₹1,54,000      │
│ [RotateCcw icon]          │ │ [Clock icon - Amber]      │ │ [Truck icon - Teal]       │ │ [CheckCircle2 - Emerald]  │
└───────────────────────────┘ └───────────────────────────┘ └───────────────────────────┘ └───────────────────────────┘
```
...
1. **Total Claims**:
   ...
   - `totalValue = claims.reduce((sum, c) => sum + (c.refundAmount || c.orderTotalAmount || 0), 0)`
2. **Pending Review**:
   - `pendingCount = claims.filter(c => c.status === 'PENDING').length`
   - Styled with `border-amber-200 bg-amber-50/70 text-amber-900`
3. **Pickups Scheduled**:
   - `scheduledCount = claims.filter(c => c.status === 'PICKUP_SCHEDULED').length`
   - Styled with `border-[#B8D8D1] bg-[#E3F0ED]/70 text-[#1E6A62]`
4. **Completed Refunds**:
   - `completedCount = claims.filter(c => c.status === 'COMPLETED').length`
   - `completedValue = claims.filter(c => c.status === 'COMPLETED').reduce((sum, c) => sum + (c.refundAmount || 0), 0)`
   - Styled with `border-emerald-200 bg-emerald-50/70 text-emerald-900`

---

### 5.3 Filterable Claims Table & Search Controls

#### Controls Header:
- **Status Filter Tabs**:
  - `ALL`: All Claims (`claims.length`)
  - `PENDING`: Pending Review (`pendingCount`)
  - `APPROVED`: Approved, Awaiting Carrier (`approvedCount`)
  - `PICKUP_SCHEDULED`: In Reverse Transit (`scheduledCount`)
  - `COMPLETED`: Completed Refunds / Exchanges (`completedCount`)
  - `REJECTED`: Disallowed (`rejectedCount`)
- **Live Search Bar**:
  - Full-text search matching:
    * Claim ID (e.g. `101` or `RET-101`)
    * Order ID (e.g. `37` or `SK-37`)
    * Customer Name (e.g. `Kalyani`)
    * Customer Email (e.g. `kalyani@example.com`)
    * Reverse AWB tracking code (e.g. `BDR-89214`)
- **Type Filter Dropdown / Pill**:
  - All Types / Returns Only / Exchanges Only

#### Table Columns:
1. **Claim ID & Type**:
   - Badge: `#RET-{claim.id}`
   - Type tag: `RETURN` (Deep Indigo) or `EXCHANGE` (Royal Violet)
   - Relative/formatted timestamp: `formatDate(claim.createdAt)`
2. **Customer & Order Details**:
   - Customer Name & Email
   - Order `#SK-{claim.orderId}` (with link/popover to order items)
   - Elapsed post-delivery tag: `{claim.daysSinceDelivery ?? '—'}d post-delivery`
3. **Reason Taxonomy**:
   - Readable badge mapping:
     * `COLOR_MISMATCH` -> "Color Mismatch"
     * `ZARI_DEFECT` -> "Zari / Weave Defect"
     * `FABRIC_FEEL` -> "Fabric Feel / Texture"
     * `INCORRECT_ITEM` -> "Incorrect Item"
     * `SIZE_MISMATCH` -> "Deficient Dimensions"
     * `OTHER` -> "Other Issue"
   - Truncated customer comment snippet with "Read more" trigger
4. **Financial Telemetry**:
   - Refund Amount: `₹{claim.refundAmount?.toLocaleString('en-IN')}`
   - Refund Mode badge:
     * `ORIGINAL_PAYMENT` -> "Source Bank/UPI"
     * `STORE_CREDIT` -> "Wallet Credit (+5%)"
     * `EXCHANGE_DRAPE` -> "Replacement Saree"
5. **Defect Photos**:
   - Horizontal thumbnail strip of up to 3 defect images.
   - Interactive zoom hover indicator with `Eye` icon.
   - Clicking thumbnail immediately triggers the **Side-by-Side Inspection Lightbox**.
6. **Reverse Courier & AWB**:
   - If `PICKUP_SCHEDULED` or `COMPLETED`:
     * Displays Courier Partner (e.g. `Blue Dart Reverse Logistics`)
     * Displays AWB badge with 1-click **Copy to Clipboard** icon button (`Copy` -> `Check`).
   - If not yet scheduled:
     * Subtle placeholder: "Awaiting pickup"
7. **Status Pill**:
   - `PENDING`: Amber pill with `Clock` icon
   - `APPROVED`: Sky pill with `ShieldCheck` icon
   - `PICKUP_SCHEDULED`: Teal/Indigo pill with `Truck` icon
   - `COMPLETED`: Emerald pill with `CheckCircle2` icon
   - `REJECTED`: Rose pill with `XCircle` icon
8. **1-Click Moderation Actions**:
   - Context-aware action buttons depending on claim status:
     - When `PENDING`:
       * `[Approve]` button (Teal/Dark)
       * `[Reject]` button (Rose outline)
     - When `APPROVED`:
       * `[Schedule Pickup]` button (Primary Teal with `Truck` icon)
       * `[Reject]` button (Rose outline)
     - When `PICKUP_SCHEDULED`:
       * `[Complete Refund]` button (Emerald with `CheckCircle2` icon)
     - When `COMPLETED`:
       * "Disbursed" badge
     - When `REJECTED`:
       * "Rejected" badge with hoverable admin note

---

### 5.4 High-Resolution Side-by-Side Inspection Lightbox

Triggered when clicking any defect thumbnail or the "Inspect Evidence" action.

#### Layout Architecture:
- Modal Dialog with backdrop blur (`bg-black/75 backdrop-blur-xs`, `role="dialog"`, `aria-modal="true"`).
- Keyboard listener: `Escape` closes modal, `ArrowLeft` / `ArrowRight` navigates photos.
- 2-Column Split Pane (`grid grid-cols-1 lg:grid-cols-12 max-w-6xl w-full`):

```
┌──────────────────────────────────────────────┬──────────────────────────────────────────┐
│ Left Pane (7 cols): Photo Gallery            │ Right Pane (5 cols): Context & Action    │
├──────────────────────────────────────────────┼──────────────────────────────────────────┤
│ [ Active High-Res Photo Display ]            │ Claim #RET-101                           │
│                                              │ Order: #SK-37 (Kanchipuram Silk Saree)   │
│                                              │ Customer: Kalyani Sundaram               │
│                                              │ Reason: Zari / Weave Defect              │
│                                              │                                          │
│                                              │ Customer Statement:                      │
│                                              │ "Frayed metallic zari threads on the     │
│                                              │ lower pallu border. Tarnished warp."     │
│                                              │                                          │
│ [Thumb 1]  [Thumb 2]  [Thumb 3]              │ Refund: ₹18,500 (Original Payment)       │
│ Photo 1 of 2 (Click thumbnail to switch)     │                                          │
...
#### Detailed Features:
- **Left Pane**:
  - Large preview container with object-contain display.
  - Image error fallback handling (displays gentle placeholder if photo fails to load).
  - Thumbnail switcher strip below the main image with active gold border indicator.
- **Right Pane**:
  - Full claim metadata, order reference, customer profile, and refund mode.
  - Verbatim customer comments box.
  - Delivery chronology: Delivered date, days elapsed, claim timestamp.
  - Embedded quick-action moderation buttons so the operator can approve, reject, or schedule pickup without returning to the table.

---

### 5.5 1-Click Moderation Controls & Workflow Modals

#### 1. Approve Return Control
- Action: Direct 1-click execution.
- Dispatches `PUT /api/admin/returns/{id}/status` with `{ status: 'APPROVED', adminNotes: 'Claim approved by staff' }`.
- Optimistically updates UI state and refreshes dataset.
- Toast feedback: `"Claim #RET-{id} has been approved for reverse pickup."`

#### 2. Assign Courier & Schedule Pickup Modal (`PickupScheduleModal`)
- Triggered by clicking "Schedule Pickup" on `APPROVED` claims.
- Dialog contents:
  - Header: `"Schedule Doorstep Reverse Pickup — Claim #RET-{claim.id}"`
  - Order Reference: `#SK-{claim.orderId}` — Customer: `{claim.customerName}`
  - **Courier Partner Selector** (Dropdown + quick chips):
    * `Blue Dart Reverse Logistics` (Default recommended)
    * `Delhivery Reverse`
    * `DTDC Express`
    * `India Post Speed Post`
    * Custom carrier input
  - **Reverse AWB Tracking Number**:
    * Smart Auto-generator: `BDR-RET-${claim.orderId}-${Date.now().toString().slice(-4)}`
    * "Regenerate Suggested AWB" button
    * Editable text input allowing manual carrier barcode scanning
  - **Pickup Notes / Instructions**:
    * Textarea pre-filled with: `"Please hand over saree in original packaging with Silk Mark tag intact."`
  - Action Buttons:
    * `Cancel` (Dismisses modal)
    * `Confirm Pickup Schedule` (Submits `PUT` request with `status: 'PICKUP_SCHEDULED'`)

#### 3. Complete Refund / Exchange Control
- Triggered by clicking "Complete Refund" on `PICKUP_SCHEDULED` claims.
- Dialog confirmation:
  - Verifies that the saree has been physically received at the central atelier and passed fabric integrity checks.
  - Displays refund disbursement amount (`₹{claim.refundAmount}`) and target mode (`ORIGINAL_PAYMENT` / `STORE_CREDIT`).
  - Action: Submits `PUT` request with `{ status: 'COMPLETED', refundAmount: claim.refundAmount, adminNotes: 'Item inspected and verified at hub. Refund finalized.' }`.

#### 4. Reject Return Modal (`RejectReturnModal`)
- Enforces backend requirement: `adminNotes` is **strictly mandatory**.
- Dialog contents:
  - Header: `"Reject Return Claim #RET-{claim.id}"`
  - Warning alert: *"This action will decline the claim. The customer will see this rejection reason on their My Orders page."*
  - **Policy Rejection Reason Quick-Chips**:
    * *"Item shows evidence of wear, perfume, or laundering."*
    * *"Authenticity security tag or Silk Mark seal removed."*
    * *"Defect not substantiated by submitted condition photographs."*
    * *"Return window exceeded 7-day post-delivery cutoff."*
    * *"Blouse piece has already been cut or altered."*
  - **Custom Justification Textarea**:
    * Clicking any policy chip automatically inserts the text into the textarea.
    * Textarea is editable for custom details.
    * Validation: Form cannot be submitted if textarea is empty or whitespace only.
  - Action Buttons:
    * `Cancel`
    * `Confirm Rejection` (Disabled if textarea is empty)

---

## 6. Complete Component Code Template for Downstream Worker

Below is the complete, production-ready implementation template for `frontend/src/pages/Admin/ManageReturns.jsx`:
          ...
          <div className="relative w-full md:w-72">
            ...
            {searchQuery && (
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 text-[#71817A] hover:text-[#17211F]"
              >
                          ...
                          <div>
                            ...
                            <div className="mt-1 flex items-center gap-1">
                              ...
                              <button
                                ...
                                className="p-1 text-[#71817A] hover:text-[#1E6A62] transition"
                                title="Copy AWB code"
                              >
                      ...
                      <td className="p-3.5 align-top text-right">
                        <div className="flex flex-col sm:flex-row items-end sm:items-center justify-end gap-1.5">
                          ...
                          <button
                            ...
                            className="p-1.5 text-[#71817A] hover:text-[#17211F] rounded-md transition"
          ...
          <div className="bg-white border border-[#DDD8CF] rounded-3xl max-w-5xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#DDD8CF] bg-[#F7F4EE]">
              ...
              <button
                ...
                className="p-1.5 text-[#71817A] hover:text-[#17211F] rounded-full hover:bg-white transition"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
                      ...
                      {inspectClaim.images.length > 1 && (
                        <>
                          <button
                            ...
                            className="absolute left-2 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/60 hover:bg-black text-white"
                          >
                          ...
                          <button
                            ...
                            className="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/60 hover:bg-black text-white"
                          >
          ...
          <div className="bg-white border border-[#DDD8CF] rounded-3xl p-6 max-w-md w-full space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#DDD8CF] pb-3">
              ...
              <button
                ...
                className="text-[#71817A] hover:text-[#17211F]"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
...
## 7. Exact File Diffs for Downstream Worker

### 7.1 Diff for `frontend/src/routes/AppRouter.jsx`
```diff
--- a/frontend/src/routes/AppRouter.jsx
+++ b/frontend/src/routes/AppRouter.jsx
@@ -46,6 +46,7 @@ const AnalyticsDashboard = lazy(() => import('../pages/Admin/AnalyticsDashboard
...
### 7.2 Diff for `frontend/src/pages/Admin/AdminDashboard.jsx`
```diff
...
## 8. Bundle Budget & Performance Compliance

1. **Chunk Budget Ceiling**:
   - Production bundle budget requirement: chunks strictly `< 500 kB`.
   - `ManageReturns.jsx` uses exclusively Tailwind utility classes and Lucide icons.
   - Built-in lightweight state management: no heavy modal libraries (pure React state), no external lightbox packages, and pure CSS/SVG telemetry.
   - Projected production chunk: **~18–24 kB** (uncompressed), **~5.5 kB** (gzipped).
2. **Offline Resilience**:
   - Built-in fallback to `MOCK_ADMIN_RETURNS` ensures that even during local integration testing or if the backend database lacks returns data, the console immediately renders with complete interactive fidelity.
3. **Accessibility**:
   - `role="dialog"`, `aria-modal="true"`, `aria-labelledby`, and `Escape` dismissal.
   - Clear semantic table structure with high color contrast meeting WCAG AA standards.

---

## 9. Verification & Acceptance Checklist for Downstream Worker

- [ ] Route `/admin/returns` opens `ManageReturns.jsx` without runtime error.
- [ ] Sidebar under `Commerce` displays "Returns & Exchanges" with `RotateCcw` icon.
- [ ] 4 KPI summary cards render accurate totals and monetary sums.
- [ ] Status tabs filter claims accurately (`ALL`, `PENDING`, `APPROVED`, `PICKUP_SCHEDULED`, `COMPLETED`, `REJECTED`).
- [ ] Search input matches claim ID, order ID, customer name, and tracking number in real time.
- [ ] Clicking defect photo thumbnails triggers the side-by-side inspection lightbox.
- [ ] 1-Click "Approve" advances status to `APPROVED`.
- [ ] "Schedule Pickup" opens modal with courier partner dropdown, auto-generated AWB, and advances status to `PICKUP_SCHEDULED`.
- [ ] "Complete Refund" advances status to `COMPLETED`.
- [ ] "Reject" modal enforces mandatory justification reason and advances status to `REJECTED`.
- [ ] Production build (`npm run build`) succeeds with all chunks under 500 kB.
